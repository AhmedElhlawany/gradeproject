...
export default function UsersBookedFlights() {
  ...
  const handleDelete = async (userId, flightIndex) => {
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`http://localhost:3000/api/users/${userId}/flights/${flightIndex}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
        ...
        prevUsers
          .map((user) =>
            user.id === userId
              ? {
                  ...user,
                  bookedFlights: user.bookedFlights.filter((_, index) => index !== flightIndex),
                }
              : user
          )
          .filter((user) => user.bookedFlights.length > 0)
